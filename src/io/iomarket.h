...
class IOMarket
{
	public:
		...
		static void createOffer(uint32_t playerId, MarketAction_t action, uint32_t itemId, uint16_t amount, uint32_t price, bool anonymous, uint16_t tier);
		static void acceptOffer(uint32_t offerId, uint16_t amount);
		static void deleteOffer(uint32_t offerId);

		static void appendHistory(uint32_t playerId, MarketAction_t type, uint16_t itemId, uint16_t amount, uint32_t price, time_t timestamp, MarketOfferState_t state, uint16_t tier);
	...
	private:
		...
		std::map<uint16_t, std::map<uint16_t, MarketStatistics>> purchaseStatistics;
		std::map<uint16_t, std::map<uint16_t, MarketStatistics>> saleStatistics;
};
