...
// Bosstiary
bool IOLoginData::loadBossSlots(Player* player)
{
  Database& db = Database::getInstance();
  DBResult_ptr result;
  std::ostringstream query;
  query << "SELECT `num`, `boss_id`, `removals` FROM `boss_slots` WHERE `player_id` = " << player->getGUID();
  if ((result = db.storeQuery(query.str()))) {
    do {
      uint16_t slotNum = result->getNumber<uint16_t>("num");
      player->bossSlots[slotNum] = result->getNumber<uint16_t>("boss_id");
      player->bossSlotsRemovals[slotNum] = result->getNumber<uint16_t>("removals");
    } while (result->next());
  }
  else {
    query.str(std::string());
    DBInsert bossSlotsQuery("INSERT INTO `boss_slots` (`player_id`, `num`, `boss_id`, `removals`) VALUES ");
    for (size_t num = 0; num < 2; num++) {
      query << player->getGUID() << ',' << num << ',' << 0 << ',' << 0;
      if (!bossSlotsQuery.addRow(query)) {
        return false;
      }
    }
    if (!bossSlotsQuery.execute()) {
      return false;
    }
    // Reload player data
    return loadBossSlots(player);
  }
...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
  ...
  player->setBossPoints(result->getNumber<uint32_t>("boss_points"));
  player->setDust(result->getNumber<int32_t>("dust"));
  player->setMaximumDust(result->getNumber<int32_t>("maximum_dust"));
...
bool IOLoginData::savePlayer(Player* player)
{
  ...
  query << "`dust` = " << player->getDust() << ',';
  query << "`maximum_dust` = " << player->getMaximumDust() << ',';
  ...
  // Bosstiary
  query.str(std::string());
  query << "DELETE FROM `boss_slots` WHERE `player_id` = " << player->getGUID();
  if (!db.executeQuery(query.str())) {
    SPDLOG_WARN("[IOLoginData::savePlayer] - Failed to delete table 'boss_slots' from player: {}", player->getName());
    return false;
  }
  query.str(std::string());
  DBInsert bossSlotsQuery("INSERT INTO `boss_slots` (`player_id`, `num`, `boss_id`, `removals`) VALUES ");
  for (size_t num = 0; num < 2; num++) {
    query << player->getGUID() << ',' << num << ',' << player->bossSlots[num] << ',' << player->bossSlotsRemovals[num];
    if (!bossSlotsQuery.addRow(query)) {
      return false;
    }
  }
  ...
  if (!bossSlotsQuery.execute()) {
    SPDLOG_WARN("[IOLoginData::savePlayer] - Failed for save bosstiary from player: {}", player->getName());
    return false;
  }
