		...
		if (result->getNumber<uint16_t>("sale") == 1) {
			...
			if (itemType.stackable) {
				...
				for (uint16_t i = 0; i < amount; ++i) {
					...
					if (result->getNumber<uint16_t>("tier") != 0)
						item->setTier(result->getNumber<uint16_t>("tier"));
					if (g_game.internalAddItem(player->getInbox(), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
						delete item;
						break;
					}
...
void IOMarket::checkExpiredOffers()
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `amount`, `price`, `itemtype`, `tier`, `player_id`, `sale` FROM `market_offers` WHERE `created` <= " << lastExpireDate;
...
MarketOfferEx IOMarket::getOfferByCounter(uint32_t timestamp, uint16_t counter)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `sale`, `itemtype`, `tier`, `amount`, `created`, `price`, `player_id`, `anonymous`, (SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` FROM `market_offers` WHERE `created` = " << created << " AND (`id` & 65535) = " << counter << " LIMIT 1";
...
void IOMarket::createOffer(uint32_t playerId, MarketAction_t action, uint32_t itemId, uint16_t amount, uint32_t price, bool anonymous, uint16_t tier)
{
	std::ostringstream query;
	query << "INSERT INTO `market_offers` (`player_id`, `sale`, `itemtype`, `tier`, `amount`, `price`, `created`, `anonymous`) VALUES (" << playerId << ',' << action << ',' << itemId << ',' << tier << ',' << amount << ',' << price << ',' << time(nullptr) << ',' << anonymous << ')';
...
void IOMarket::appendHistory(uint32_t playerId, MarketAction_t type, uint16_t itemId, uint16_t amount, uint32_t price, time_t timestamp, MarketOfferState_t state, uint16_t tier)
{
	std::ostringstream query;
...
bool IOMarket::moveOfferToHistory(uint32_t offerId, MarketOfferState_t state)
{
	...
	appendHistory(result->getNumber<uint32_t>("player_id"), static_cast<MarketAction_t>(result->getNumber<uint16_t>("sale")), result->getNumber<uint16_t>("itemtype"), result->getNumber<uint16_t>("amount"), result->getNumber<uint32_t>("price"), result->getNumber<uint32_t>("created") + marketOfferDuration, state, result->getNumber<uint16_t>("tier"));
	return true;
}
...
void IOMarket::updateStatistics()
{
	...
	do {
		...
		if (result->getNumber<uint16_t>("sale") == MARKETACTION_BUY) {
			statistics = &purchaseStatistics[result->getNumber<uint16_t>("itemtype")][result->getNumber<uint16_t>("tier")];
		} else {
			statistics = &saleStatistics[result->getNumber<uint16_t>("itemtype")][result->getNumber<uint16_t>("tier")];
		}
