...
private:
	...
	void sendBosstiaryList();
	void sendBosstiaryBossSlots();
	void parseBosstiaryAction(NetworkMessage& msg);
	void sendBosstiaryBasicData();
	...
	void sendMarketBrowseItem(uint16_t itemId, const MarketOfferList& buyOffers, const MarketOfferList& sellOffers, uint16_t tier);
	...
	void sendMarketDetail(uint16_t itemId, uint16_t tier);
	...
	void addCreatureIcons(NetworkMessage& msg, std::vector<CreatureIcon> icons);
