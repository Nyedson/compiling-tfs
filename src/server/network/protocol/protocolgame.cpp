...
void ProtocolGame::AddItem(NetworkMessage &msg, const Item *item)
{
	...
	else if (version >= 1200 && it.isContainer() && player->getOperatingSystem() <= CLIENTOS_NEW_MAC)
	{
    	...
    	if (container && item->getWeaponType() == WEAPON_QUIVER && player->getThing(CONST_SLOT_RIGHT) == item) {
      		...
      		msg.addByte(0x00);
	} else if (version >= 1280 && it.classification > 0) {
		msg.addByte(static_cast<uint8_t>(item->getTier())); // item tier
	} else if (version >= 1290 && it.showInfo) {
		if (it.stopTime || it.decayTime > 0 || it.decayTo != -1) {
...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	if (m_spectator) {
		...
		case 0x1D: addGameTask(&Game::playerReceivePingBack, player->getID()); break; //testing cast 
...
void ProtocolGame::parsePacketFromDispatcher(NetworkMessage msg, uint8_t recvbyte)
{
	...
	if (!player || player->isRemoved() || player->getHealth() <= 0) {
		...
		switch (recvbyte) {
			...
			case 0xAE: sendBosstiaryList(); break;
			case 0xAF: sendBosstiaryBossSlots(); break;
			case 0xB0: parseBosstiaryAction(msg); break;
...
void ProtocolGame::parseSay(NetworkMessage &msg)
{
	...
	if (m_spectator) {
		g_dispatcher.addTask(createTask(std::bind(&Spectators::handle, player->client, player, getThis(), text, channelId)));
		return;
	}
...
void ProtocolGame::parseMarketBrowse(NetworkMessage &msg)
{
	if (version >= 1280) {
		...
		if (type == MARKETREQUEST_OWN_OFFERS) {
			addGameTask(&Game::playerBrowseMarketOwnOffers, player->getID());
		} else if (type == MARKETREQUEST_OWN_HISTORY) {
			...
			if (it.classification != 0)
				tier = static_cast<uint16_t>(msg.getByte());
		...
		if (browseId == MARKETREQUEST_OLD_OWN_OFFERS)
		{
			addGameTask(&Game::playerBrowseMarketOwnOffers, player->getID());
		}
		else if (browseId == MARKETREQUEST_OLD_OWN_HISTORY)
		{
			addGameTask(&Game::playerBrowseMarketOwnHistory, player->getID());
		}
		else
...
void ProtocolGame::parseMarketCreateOffer(NetworkMessage& msg)
{
	...
	uint16_t spriteId = msg.get<uint16_t>();
	uint16_t tier = 0;
	if (version >= 1280 && Item::items.getItemIdByClientId(spriteId).classification != 0)
		tier = static_cast<uint16_t>(msg.getByte());
	uint16_t amount = msg.get<uint16_t>();
	uint32_t price = 0;
	if (version >= 1280)
		...
		price = msg.get<uint32_t>();
	bool anonymous = (msg.getByte() != 0);
	if (amount > 0 && price > 0) {
		g_game.playerCreateMarketOffer(player->getID(), type, spriteId, amount, price, anonymous, tier);
	}
}
...
void ProtocolGame::sendCreatureIcons(const Creature* creature)
{
	if (!creature || version < 1260)
		return;
	NetworkMessage msg;
	msg.addByte(0x8B);
	msg.add<uint32_t>(creature->getID());
	msg.addByte(14); // type 14 for this
	addCreatureIcons(msg, creature->getIcons());

	writeToOutputBuffer(msg);
}

void ProtocolGame::addCreatureIcons(NetworkMessage& msg, std::vector<CreatureIcon> icons){
	msg.addByte(icons.size());
	for (CreatureIcon icon : icons) {
		msg.addByte(icon.id);
		msg.addByte(icon.playerIcon ? 0x00 : 0x01);
		msg.add<uint16_t>(icon.value);
	}
}
...
void ProtocolGame::sendCyclopediaCharacterGeneralStats()
{
	...
	if (version >= 1270) {
		...
		//5 holy
		//6 death
		//7 heal -- have more than 7, drowing and others that idk
		int loop = 0;
		msg.addByte(loop); //
		if (loop == 1) {
			msg.addByte(0); //type
			msg.add<uint16_t>(0); //value
		}
	}
...
void ProtocolGame::sendChannelMessage(const std::string &author, const std::string &text, SpeakClasses type, uint16_t channel, uint32_t spectatorLevel)
{
	NetworkMessage msg;
	msg.addByte(0xAA);
	msg.add<uint32_t>(0x00);
	msg.addString(author);
	msg.add<uint16_t>(spectatorLevel);
	msg.addByte(type);
...
void ProtocolGame::sendMarketEnter(uint32_t depotId)
{
	...
	std::map<uint16_t, std::map<uint16_t, uint32_t>> classificationItems;
	...
	do {
		...
		for (Item* item : container->getItemList()) {
			...
			if (itemType.classification != 0) {
				classificationItems[itemType.wareId][item->getTier()] += Item::countByType(item, -1);
			} else {
	...
	for (std::map<uint16_t, std::map<uint16_t, uint32_t>>::const_iterator it = classificationItems.begin(); it != classificationItems.end(); ++it) {
		for (std::map<uint16_t, uint32_t>::const_iterator it2 = it->second.begin(); it2 != it->second.end(); ++it2) {
			if (version >= 1280 || it2->first == 0) {
				msg.add<uint16_t>(it->first);
				if (version >= 1280)
					msg.addByte(static_cast<uint8_t>(it2->first));
				msg.add<uint16_t>(std::min<uint32_t>(0xFFFF, it2->second));
				itemsToSend2++;
			}
		}
	}
	...
	msg.add<uint16_t>(itemsToSend + itemsToSend2);
...
void ProtocolGame::sendMarketBrowseItem(uint16_t itemId, const MarketOfferList& buyOffers, const MarketOfferList& sellOffers, uint16_t tier)
{
	NetworkMessage msg;

	msg.addByte(0xF9);
	if (version >= 1280)
		msg.addByte(MARKETREQUEST_BROWSE_ITEM);
	msg.addItemId(itemId);

	if (version >= 1280 && Item::items[itemId].classification > 0) {
		msg.addByte(static_cast<uint8_t>(tier));
	}
...
void ProtocolGame::sendMarketAcceptOffer(const MarketOfferEx& offer)
{
	...
	if (version >= 1280 && Item::items[offer.itemId].classification > 0) {
		msg.addByte(static_cast<uint8_t>(offer.tier));
	}
...
void ProtocolGame::sendMarketBrowseOwnOffers(const MarketOfferList& buyOffers, const MarketOfferList& sellOffers)
{
	...
	if (version >= 1280)
		...
		msg.add<uint16_t>(MARKETREQUEST_OLD_OWN_OFFERS);
...
void ProtocolGame::sendMarketCancelOffer(const MarketOfferEx& offer)
{
	...
	if (offer.type == MARKETACTION_BUY) {
		...
		msg.add<uint16_t>(offer.counter);
		msg.addItemId(offer.itemId);
		if (Item::items[offer.itemId].classification != 0 && version >= 1200)
			msg.addByte(offer.tier);
		msg.add<uint16_t>(offer.amount);
		if (version >= 1280)
			msg.add<uint64_t>(offer.price);
...
void ProtocolGame::sendMarketBrowseOwnHistory(const HistoryMarketOfferList& buyOffers, const HistoryMarketOfferList& sellOffers)
{
	...
	if (version >= 1280)
		...
		msg.add<uint16_t>(MARKETREQUEST_OLD_OWN_HISTORY);
...
void ProtocolGame::sendForgingData()
{
	...
	msg.addByte(0x32);
	msg.addByte(0x4B);
	msg.addByte(static_cast<uint8_t>(player->getMaximumDust()));
	msg.addByte(0xE1);
	msg.addByte(0x64);
	msg.addByte(0x64);
	msg.addByte(0x32);
...
void ProtocolGame::sendBosstiaryList()
{
	NetworkMessage msg;
	msg.addByte(0x73);
	std::map<uint16_t, std::string> bossList = g_game.getBosstiaryList();
	msg.add<uint16_t>(bossList.size());
	for (auto it = bossList.begin(); it != bossList.end(); it++) {
		msg.add<uint32_t>(it->first);
		msg.addByte(g_monsters.getMonsterType(it->second)->info.bosstiaryCategory);
		msg.add<uint32_t>(player->getBosstiaryKillCount(it->first));
		msg.addByte(0x00);
	}
	writeToOutputBuffer(msg);
}

void ProtocolGame::parseBosstiaryAction(NetworkMessage& msg)
{
	if (!player)
		return;
	uint16_t slot = msg.getByte();
	uint32_t bossPoints = player->getBossPoints();
	if ((slot == 1 && bossPoints == 0) || (slot == 2 && bossPoints < BOSSTIARY_CONF_MIN_BOSS_POINTS_SECOND_SLOT) || (slot != 1 && slot != 2))
		return;
	if (player->bossSlots[slot-1] == 0) {
		uint32_t bossId = msg.get<uint32_t>();
		// Choosing
		if (bossId == 0)
			return; // Shouldn't happen
		auto bosstiaryList = g_game.getBosstiaryList();
		auto it = bosstiaryList.find(bossId);
		if (it == bosstiaryList.end())
			return;
		MonsterType* mType = g_monsters.getMonsterType(it->second);
		if (!mType)
			return;
		uint32_t kills = player->getBosstiaryKillCount(bossId);
		if ((mType->info.bosstiaryCategory == BOSSTIARY_CATEGORY_BANE && kills < BOSSTIARY_KILLS_BANE_PROWESS) ||
			(mType->info.bosstiaryCategory == BOSSTIARY_CATEGORY_ARCHFOE && kills < BOSSTIARY_KILLS_ARCHFOE_PROWESS) ||
			(mType->info.bosstiaryCategory == BOSSTIARY_CATEGORY_NEMESIS && kills < BOSSTIARY_KILLS_NEMESIS_PROWESS))
			return;
		player->setBossSlotRaceId(slot-1, bossId);
		sendBosstiaryBossSlots();
	} else {
		IOBosstiary g_bosstiary;
		uint32_t actualBossId = player->getBossSlotRaceId(slot-1);
		if (actualBossId == 0)
			return;
		uint32_t cost = 0;
		if (g_monsters.getMonsterTypeByBossId(actualBossId)->name == g_game.getBoostedBossName()) {
			cost = 0;
		} else {
			cost = g_bosstiary.getRemovalCost(player->getBossSlotRemovals(slot-1));
		}
		if (g_game.removePlayerSlotBoss(player, slot-1, cost)) {
			sendBosstiaryBossSlots();
		}
	}
}

void ProtocolGame::sendBosstiaryBossSlots()
{
	sendResourcesBalance(player->getMoney(), player->getBankBalance());
	IOBosstiary g_bosstiary;
	NetworkMessage msg;
	msg.addByte(0x62);
	uint32_t bossPoints = player->getBossPoints();
	msg.add<uint32_t>(bossPoints);
	msg.add<uint32_t>(g_bosstiary.getBossPointsForLevel(g_bosstiary.getLevelByBossPoints(bossPoints) + 1));
	uint16_t level = g_bosstiary.getLevelByBossPoints(bossPoints);
	msg.add<uint16_t>(BOSSTIARY_CONF_BASE_EQUIPMENT_BONUS + level);
	msg.add<uint16_t>(BOSSTIARY_CONF_BASE_EQUIPMENT_BONUS + level + 1);
	msg.addByte(bossPoints == 0 ? 0x00 : 0x01);
	uint16_t raceId = player->getBossSlotRaceId(0);
	auto bosstiaryList = g_game.getBosstiaryList();
	msg.add<uint32_t>(raceId);
	if (raceId != 0) {
		MonsterType* mtype = g_monsters.getMonsterTypeByBossId(raceId);
		if (!mtype)
			return;
		BosstiaryCategory_t category = mtype->info.bosstiaryCategory;
		msg.addByte(category);
		uint32_t kills = player->getBosstiaryKillCount(raceId);
		msg.add<uint32_t>(kills);
		if (g_bosstiary.getBosstiaryProgress(player, kills, category) == BOSSTIARY_PROGRESS_MASTERY)
			msg.add<uint16_t>(BOSSTIARY_CONF_EXTRA_EQUIPMENT_BONUS_MASTERY + BOSSTIARY_CONF_BASE_EQUIPMENT_BONUS + level);
		else
			msg.add<uint16_t>(BOSSTIARY_CONF_BASE_EQUIPMENT_BONUS + level);
		msg.addByte(0x00);
		msg.addByte(0x00);
		bool boosted = g_game.getBoostedBossName() == mtype->name;
		msg.add<uint32_t>(boosted ? 0 : g_bosstiary.getRemovalCost(player->getBossSlotRemovals(0)));
		msg.addByte(boosted ? 0x01 : 0x00);
	}
	if (bossPoints < BOSSTIARY_CONF_MIN_BOSS_POINTS_SECOND_SLOT) {
		msg.addByte(0x00);
		msg.add<uint32_t>(BOSSTIARY_CONF_MIN_BOSS_POINTS_SECOND_SLOT);
	} else {
		raceId = player->getBossSlotRaceId(1);
		msg.add<uint32_t>(raceId);
		MonsterType* mtype = g_monsters.getMonsterTypeByBossId(raceId);
		if (!mtype)
			return;
		...
		msg.addByte(0x00);
		bool boosted = g_game.getBoostedBossName() == mtype->name;
		msg.add<uint32_t>(boosted ? 0 : g_bosstiary.getRemovalCost(player->getBossSlotRemovals(1)));
		msg.addByte(boosted ? 0x01 : 0x00);
	}
	msg.addByte(0x01);
	MonsterType* mtypeBoosted = g_monsters.getMonsterType(g_game.getBoostedBossName());
	msg.add<uint32_t>(mtypeBoosted->info.bossid);
	msg.addByte(mtypeBoosted->info.bosstiaryCategory);
	msg.add<uint32_t>(player->getBosstiaryKillCount(mtypeBoosted->info.bossid));
	msg.add<uint16_t>(BOSSTIARY_CONF_BOOSTED_BOSS_EQUIPMENT_BONUS);
	msg.addByte(BOSSTIARY_CONF_BOOSTED_BOSS_KILL_COUNT);
	msg.addByte(0x00);
	msg.add<uint32_t>(0);
	msg.addByte(0x01);
	auto bufferPosition = msg.getBufferPosition();
	msg.skipBytes(3);
	uint16_t total = 0;
	for (auto it = bosstiaryList.begin(); it != bosstiaryList.end(); ++it) {
		MonsterType* mType = g_monsters.getMonsterType(it->second);
		if (g_bosstiary.getBosstiaryProgress(player, player->getBosstiaryKillCount(it->first), mType->info.bosstiaryCategory) >= BOSSTIARY_PROGRESS_PROWESS) {
			total++;
			msg.add<uint32_t>(it->first);
			msg.addByte(mType->info.bosstiaryCategory);
		}
	}
	msg.setBufferPosition(bufferPosition);
	if (total == 0) {
...
void ProtocolGame::sendBosstiaryBasicData()
{
	NetworkMessage msg;
	msg.addByte(0x61);
	msg.add<uint16_t>(BOSSTIARY_KILLS_BANE_PROWESS);
	msg.add<uint16_t>(BOSSTIARY_KILLS_BANE_EXPERTISE);
	msg.add<uint16_t>(BOSSTIARY_KILLS_BANE_MASTERY);
	msg.add<uint16_t>(BOSSTIARY_KILLS_ARCHFOE_PROWESS);
	msg.add<uint16_t>(BOSSTIARY_KILLS_ARCHFOE_EXPERTISE);
	msg.add<uint16_t>(BOSSTIARY_KILLS_ARCHFOE_MASTERY);
	msg.add<uint16_t>(BOSSTIARY_KILLS_NEMESIS_PROWESS);
	msg.add<uint16_t>(BOSSTIARY_KILLS_NEMESIS_EXPERTISE);
	msg.add<uint16_t>(BOSSTIARY_KILLS_NEMESIS_MASTERY);
	msg.add<uint16_t>(BOSSTIARY_POINTS_BANE_PROWESS);
	msg.add<uint16_t>(BOSSTIARY_POINTS_BANE_EXPERTISE);
	msg.add<uint16_t>(BOSSTIARY_POINTS_BANE_MASTERY);
	msg.add<uint16_t>(BOSSTIARY_POINTS_ARCHFOE_PROWESS);
	msg.add<uint16_t>(BOSSTIARY_POINTS_ARCHFOE_EXPERTISE);
	msg.add<uint16_t>(BOSSTIARY_POINTS_ARCHFOE_MASTERY);
	msg.add<uint16_t>(BOSSTIARY_POINTS_NEMESIS_PROWESS);
	msg.add<uint16_t>(BOSSTIARY_POINTS_NEMESIS_EXPERTISE);
	msg.add<uint16_t>(BOSSTIARY_POINTS_NEMESIS_MASTERY);
	writeToOutputBuffer(msg);
}

void ProtocolGame::sendMarketDetail(uint16_t itemId, uint16_t tier)
{
	NetworkMessage msg;
	msg.addByte(0xF8);
	msg.addItemId(itemId);
	...
	if (version >= 1270) {
		...
		if (version >= 1280) {
			if (it.classification != 0) {
				strstream.str("");
				strstream << it.classification;
				msg.addString(strstream.str());
				strstream.str("");
				strstream << tier;
				if (tier > 0) {
					strstream << " (";
					if (it.slotPosition & SLOTP_HEAD) {
						strstream << getMomentumValue(tier)/100 << "% Momentum";
					} else if (it.slotPosition & SLOTP_ARMOR) {
						strstream << getRuseValue(tier)/100 << "% Ruse";
					} else if (it.weaponType > 0) {
						strstream << getOnSlaughtValue(tier)/100 << "\% Onslaught";
					}
					strstream << ')';
				}
				msg.addString(strstream.str());
...
void ProtocolGame::AddCreature(NetworkMessage &msg, const Creature *creature, bool known, uint32_t remove)
{
	...
	msg.addByte(player->getSkullClient(creature));
	msg.addByte(player->getPartyShield(otherPlayer)); //FUNCIONO
	...
	if (version >= 1100 && creatureType == CREATURETYPE_MONSTER)
	{
		const Creature *master = creature->getMaster();
		if (master)
		{
	...
	if (version >= 1200 && creatureType == CREATURETYPE_SUMMON_PLAYER) //mudei pra 1100 de 1200
	{
		const Creature *master = creature->getMaster();
		if (master)
		{
...
void ProtocolGame::sendImbuementWindow(Item *item)
{
	...
	if (version >= 1280 && Item::items[item->getID()].classification > 0) {
...
void ProtocolGame::sendItemsPrice()
{
	...
	if (g_game.getItemsPriceCount() > 0)
	{
		...
		for (const auto &it : items)
		{
			...
			if (version >= 1280 && Item::items[it.first].classification > 0)
			{
				msg.addByte(0);
			}
			if (version >= 1280)
				msg.add<uint64_t>(it.second);
			else
...
void ProtocolGame::spectatorLogin(const std::string& name, const std::string& password)
{
	...
	Player* foundPlayer = g_game.getPlayerByName(streamerName);
...
void ProtocolGame::castNavigation(uint16_t direction)
{
	...
	const size_t& playerSize = players.size() - 1;
	if (playerSize > 0) {
		switch (direction) {
		case DIRECTION_NORTH:
			castPosition = castPosition == 0 ? playerSize : 0;
			break;
		case DIRECTION_EAST:
			castPosition = castPosition >= playerSize ? 0 : ++castPosition;
			break;
		case DIRECTION_SOUTH:
			castPosition = castPosition >= playerSize ? 0 : playerSize;
			break;
		case DIRECTION_WEST:
			castPosition = castPosition == 0 ? playerSize : --castPosition;
			break;
		}
	}

	// extra protection (avoid out of range)
	if (castPosition > playerSize) {
		castPosition = 0;
	}
