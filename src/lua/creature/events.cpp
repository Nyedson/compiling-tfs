...
void Events::eventCreatureOnDrainHealth(Creature* creature, Creature* attacker, CombatType_t& typePrimary, int32_t& damagePrimary, CombatType_t& typeSecondary, int32_t& damageSecondary, TextColor_t& colorPrimary, TextColor_t& colorSecondary, std::string& exString)
{
	...
	LuaScriptInterface::pushString(L, exString);

	if (scriptInterface.protectedCall(L, 9, 7) != 0) {
		LuaScriptInterface::reportError(nullptr, LuaScriptInterface::popString(L));
	} else {
		typePrimary = LuaScriptInterface::getNumber<CombatType_t>(L, -7);
		damagePrimary = LuaScriptInterface::getNumber<int32_t>(L, -6);
		typeSecondary = LuaScriptInterface::getNumber<CombatType_t>(L, -5);
		damageSecondary = LuaScriptInterface::getNumber<int32_t>(L, -4);
		colorPrimary = LuaScriptInterface::getNumber<TextColor_t>(L, -3);
		colorSecondary = LuaScriptInterface::getNumber<TextColor_t>(L, -2);
		exString = LuaScriptInterface::getString(L, -1);
		lua_pop(L, 7);
	}
