...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(CONDITION_CREATUREICON)
	...
	registerEnum(CONDITION_PARAM_CREATUREICON_ID)
	registerEnum(CONDITION_PARAM_CREATUREICON_PLAYERICON)
	registerEnum(CONDITION_PARAM_CREATUREICON_VALUE)
  ...
  registerEnum(CONST_ME_ROOTS)
	...
	registerEnum(BOSSTIARY_CONF_BASE_EQUIPMENT_BONUS)
	registerEnum(BOSSTIARY_CONF_BOOSTED_BOSS_EQUIPMENT_BONUS)
	registerEnum(BOSSTIARY_CONF_BOOSTED_BOSS_KILL_COUNT)
	registerEnum(BOSSTIARY_CONF_MIN_BOSS_POINTS_SECOND_SLOT)
	registerEnum(BOSSTIARY_CONF_EXTRA_EQUIPMENT_BONUS_MASTERY)
	registerEnum(BOSSTIARY_KILLS_BANE_PROWESS)
	registerEnum(BOSSTIARY_KILLS_BANE_MASTERY)
	registerEnum(BOSSTIARY_KILLS_ARCHFOE_PROWESS)
	registerEnum(BOSSTIARY_KILLS_ARCHFOE_MASTERY)
	registerEnum(BOSSTIARY_KILLS_NEMESIS_PROWESS)
	registerEnum(BOSSTIARY_KILLS_NEMESIS_MASTERY)
	registerEnum(BOSSTIARY_CATEGORY_NONE)
	registerEnum(BOSSTIARY_CATEGORY_BANE)
	registerEnum(BOSSTIARY_CATEGORY_ARCHFOE)
	registerEnum(BOSSTIARY_CATEGORY_NEMESIS)
  ...
  registerEnum(WEAPON_QUIVER)
	...
	registerMethod("Game", "getBoostedBoss", LuaScriptInterface::luaGameGetBoostedBoss);
	
	registerMethod("Game", "getBestiaryList", LuaScriptInterface::luaGameGetBestiaryList);

	registerMethod("Game", "getBosstiaryList", LuaScriptInterface::luaGameGetBosstiaryList);
	
	registerMethod("Game", "createItemClassification", LuaScriptInterface::luaGameCreateItemClassification);

	registerMethod("Game", "getPlayers", LuaScriptInterface::luaGameGetPlayers);
	registerMethod("Game", "getRandomMonster", LuaScriptInterface::luaGameGetRandomMonster);
	...
	registerClass("ItemClassification", "", LuaScriptInterface::luaItemClassificationCreate);
	registerMetaMethod("ItemClassification", "__eq", LuaScriptInterface::luaUserdataCompare);

	registerMethod("ItemClassification", "addTier", LuaScriptInterface::luaItemClassificationAddTier);
	...
	registerMethod("Player", "getBossSlot", LuaScriptInterface::luaPlayerGetBossSlot);
	registerMethod("Player", "getBosstiaryKills", LuaScriptInterface::luaPlayerGetBosstiaryKills);
	registerMethod("Player", "addBosstiaryKill", LuaScriptInterface::luaPlayerAddBosstiaryKill);
	registerMethod("Player", "getLevelByBossPoints", LuaScriptInterface::luaPlayerGetLevelByBossPoints);
	registerMethod("Player", "getBossPoints", LuaScriptInterface::luaPlayerGetBossPoints);
	registerMethod("Player", "setBossPoints", LuaScriptInterface::luaPlayerSetBossPoints);
	
	registerMethod("Player", "getBestiaryKills", LuaScriptInterface::luaPlayerGetBestiaryKills);
	...
	registerMethod("Player", "getDust", LuaScriptInterface::luaPlayerGetDust);
	registerMethod("Player", "setDust", LuaScriptInterface::luaPlayerSetDust);

	registerMethod("Player", "getMaximumDust", LuaScriptInterface::luaPlayerGetMaximumDust);
	registerMethod("Player", "setMaximumDust", LuaScriptInterface::luaPlayerSetMaximumDust);
	...
	registerMethod("Monster", "isFromRaid", LuaScriptInterface::luaMonsterIsFromRaid);

	registerMethod("Monster", "hasFoughtSinceSpawn", LuaScriptInterface::luaMonsterHasFoughtSinceSpawn);
	
	registerMethod("Monster", "getInfluencedLevel", LuaScriptInterface::luaMonsterGetInfluencedLevel);
	registerMethod("Monster", "setInfluencedLevel", LuaScriptInterface::luaMonsterSetInfluencedLevel);
  ...
  registerMethod("Guild", "setBankBalance", LuaScriptInterface::luaGuildSetBankBalance);
	...
	registerMethod("MonsterType", "bossId", LuaScriptInterface::luaMonsterTypeBossid);
	registerMethod("MonsterType", "bosstiaryCategory", LuaScriptInterface::luaMonsterTypeBosstiaryCategory);
...
int LuaScriptInterface::luaGameGetBoostedBoss(lua_State* L)
{
	// Game.getBoostedBoss()
	pushString(L, g_game.getBoostedBossName());
	return 1;
}
...
int LuaScriptInterface::luaGameGetBestiaryList(lua_State* L)
{
   ...
   if (lua_gettop(L) <= 2) {
	...
	if (isNumber(L, 2)) {
	   std::map<uint16_t, std::string> tmplist = g_bestiary.findRaceByName("TFS", false, getNumber<BestiaryType_t>(L, 2));
...
int LuaScriptInterface::luaGameGetBosstiaryList(lua_State* L)
{
	// Game.getBosstiaryList()
	auto c_list = g_game.getBosstiaryList();
	lua_newtable(L);
...
int LuaScriptInterface::luaGameGetRandomMonster(lua_State* L)
{
	// Game.getRandomMonster()
	Monster* randomMonster = g_game.getRandomMonster();
	if (!randomMonster)
		lua_pushnil(L);
	else {
		pushUserdata<Monster>(L, randomMonster);
		setMetatable(L, -1, "Monster");
	}
	return 1;
}

int LuaScriptInterface::luaGameCreateItemClassification(lua_State* L) {
...
int LuaScriptInterface::luaPlayerGetBestiaryKills(lua_State* L)
{
	// player:getBestiaryKills(name)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		MonsterType* mtype = g_monsters.getMonsterType(getString(L, 2));
		if (mtype) {
			lua_pushnumber(L, player->getBestiaryKillCount(mtype->info.raceid));
		}
		else {
			lua_pushnil(L);
		}
...
int LuaScriptInterface::luaPlayerGetBossSlot(lua_State* L)
{
	// player:getBossSlot(slot)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		...
		if (slot > 1)
			...
			lua_pushnumber(L, player->getBossSlotRaceId(slot));
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerGetBosstiaryKills(lua_State* L)
{
	...
	if (player) {
		MonsterType* mtype = g_monsters.getMonsterType(getString(L, 2));
		if (mtype) {
			lua_pushnumber(L, player->getBosstiaryKillCount(mtype->info.bossid));
		}
		else {
			lua_pushnil(L);
		}
...
int LuaScriptInterface::luaPlayerGetLevelByBossPoints(lua_State* L)
{
	// player:getLevelByBossPoints()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
			IOBosstiary g_bosstiary;
			lua_pushnumber(L, g_bosstiary.getLevelByBossPoints(player->getBossPoints()));
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerGetBossPoints(lua_State* L)
{
	// player:getBossPoints()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getBossPoints());
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerSetBossPoints(lua_State* L)
{
...
int LuaScriptInterface::luaPlayerAddBosstiaryKill(lua_State* L)
{
	...
	if (player) {
		...
		if (mtype) {
			IOBosstiary g_bosstiary;
			g_bosstiary.addBosstiaryKill(player, mtype, getNumber<uint32_t>(L, 3, 1));
			pushBoolean(L, true);
		}
		else {
			lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerGetDust(lua_State* L)
{
	// player:getDust()
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
	...
	lua_pushnumber(L, player->getDust());
	return 1;
}

int LuaScriptInterface::luaPlayerSetDust(lua_State* L)
{
	// player:setDust(value)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	}
	int32_t value = getNumber<int32_t>(L, 2, 0);
	player->setDust(value);
	pushBoolean(L, true);
	return 1;
}

int LuaScriptInterface::luaPlayerGetMaximumDust(lua_State* L)
{
	// player:getMaximumDust()
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	}
	lua_pushnumber(L, player->getMaximumDust());
	return 1;
}

int LuaScriptInterface::luaPlayerSetMaximumDust(lua_State* L)
{
	// player:setMaximumDust(value)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	}
	int32_t value = getNumber<int32_t>(L, 2, 100);
	player->setMaximumDust(std::max<int32_t>(100, value));
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaMonsterIsFromRaid(lua_State* L)
{
	// monster:isFromRaid()
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
		pushBoolean(L, monster->isFromRaid());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaMonsterHasFoughtSinceSpawn(lua_State* L)
{
	// monster:hasFoughtSinceSpawn()
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
		pushBoolean(L, monster->hasFoughtSinceSpawn());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaMonsterGetInfluencedLevel(lua_State* L)
{
	// monster:getInfluencedLevel()
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
		lua_pushnumber(L, monster->getInfluencedLevel());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaMonsterSetInfluencedLevel(lua_State* L)
{
	// monster:setInfluencedLevel(value)
	Monster* monster = getUserdata<Monster>(L, 1);
	int32_t value = getNumber<int32_t>(L, 2, 0);
	if (monster) {
		monster->setInfluencedLevel(value);
...
int LuaScriptInterface::luaMonsterTypeBossid(lua_State* L)
{
	// get: monsterType:bossId() set: monsterType:bossId(id)
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			lua_pushnumber(L, monsterType->info.bossid);
		}
		else {
			monsterType->info.bossid = getNumber<uint16_t>(L, 2);
			g_game.addBosstiaryList(getNumber<uint16_t>(L, 2), monsterType->name);
			pushBoolean(L, true);
		}
	}
	else {
...
int LuaScriptInterface::luaMonsterTypeBosstiaryCategory(lua_State* L)
{
	// get: monsterType:bosstiaryCategory() set: monsterType:bosstiaryCategory(category)
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			lua_pushnumber(L, monsterType->info.bosstiaryCategory);
		}
		else {
			monsterType->info.bosstiaryCategory = getNumber<BosstiaryCategory_t>(L, 2);
			pushBoolean(L, true);
		}
	}
	else {
