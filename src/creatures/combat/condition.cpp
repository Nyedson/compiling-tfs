...
Condition* Condition::createCondition(ConditionId_t id, ConditionType_t type, int32_t ticks, int32_t param/* = 0*/, bool buff/* = false*/, uint32_t subId/* = 0*/)
{
	switch (type) {
		...
		case CONDITION_CREATUREICON:
			return new ConditionCreatureIcon(id, type, ticks, buff, subId);
...
void ConditionAttributes::updateBuffs(Creature* creature)
{
  ...
  if (creature->getMonster() && needUpdate) {
    g_game.updateCreatureIcons(creature);
  }
}
...
bool ConditionCreatureIcon::startCondition(Creature* creature)
{
	if (!Condition::startCondition(creature)) {
		return false;
	}
	creature->addIcon(creatureIcon);
	g_game.updateCreatureIcons(creature);
	return true;
}

void ConditionCreatureIcon::endCondition(Creature* creature)
{
	creature->removeIcon(creatureIcon);
	g_game.updateCreatureIcons(creature);
}

void ConditionCreatureIcon::addCondition(Creature* creature, const Condition* addCondition)
{
	endCondition(creature);
	setTicks(addCondition->getTicks());

	const ConditionCreatureIcon& conditionCreatureIcon = static_cast<const ConditionCreatureIcon&>(*addCondition);
	creature->removeIcon(creatureIcon);
	creatureIcon = conditionCreatureIcon.creatureIcon;
	creature->addIcon(creatureIcon);
	g_game.updateCreatureIcons(creature);
}

bool ConditionCreatureIcon::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	if (attr == CONDITIONATTR_CREATUREICON) {
		return propStream.read<CreatureIcon>(creatureIcon);
	}
	return Condition::unserializeProp(attr, propStream);
}

void ConditionCreatureIcon::serialize(PropWriteStream& propWriteStream)
{
	Condition::serialize(propWriteStream);

	propWriteStream.write<uint8_t>(CONDITIONATTR_CREATUREICON);
	propWriteStream.write<CreatureIcon>(creatureIcon);
}

bool ConditionCreatureIcon::setParam(ConditionParam_t param, int32_t value)
{
	bool ret = Condition::setParam(param, value);

	switch (param) {
		case CONDITION_PARAM_CREATUREICON_ID:
			creatureIcon.id = value;
			return true;
		case CONDITION_PARAM_CREATUREICON_PLAYERICON:
			creatureIcon.playerIcon = value == 1;
			return true;
		case CONDITION_PARAM_CREATUREICON_VALUE:
			creatureIcon.value = value;
			return true;
		default:
			return ret;
	}
