...
void Combat::CombatFunc(Creature* caster, const Position& pos, const AreaCombat* area, const CombatParams& params, CombatFunction func, CombatDamage* data)
{
    ...
    if(data) {
		...
		tmpDamage.onslaught = data->onslaught;
    }
...
void Combat::doCombatHealth(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
	...
	if (!damage.extension) {
		if (caster && caster->getPlayer()) {
			uint16_t critChance = caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_CHANCE);
			...
			if (target && target->getMonster()) {
				...
				if (playerCharmRaceid != 0) {
					...
					if (mType && playerCharmRaceid == mType->info.raceid) {
						...
						if (charm) {
							critChance += charm->percent;
						}
					}
				}
			}
			int32_t onslaughtChance = caster->getPlayer()->getOnslaughtPercent();
			if (damage.primary.type != COMBAT_HEALING && onslaughtChance != 0 && uniform_random(1, 10000) <= onslaughtChance) {
				totalDamage += 60;
				damage.onslaught = true;
			}
			if (damage.primary.type != COMBAT_HEALING && critChance != 0 && uniform_random(1, 100) <= critChance) {
				damage.critical = true;
				totalDamage += caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_DAMAGE);
			}
			if (damage.critical || damage.onslaught) {
				damage.primary.value += (damage.primary.value * totalDamage / 100);
				damage.secondary.value += (damage.secondary.value * totalDamage / 100);
			}
		}
	}
	if (canCombat) {
