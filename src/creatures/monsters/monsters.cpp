...
MonsterType* Monsters::getMonsterTypeByBossId(uint16_t thisboss) {
	std::map<uint16_t, std::string> bossid_list = g_game.getBosstiaryList();
	auto it = bossid_list.find(thisboss);
	if (it == bossid_list.end()) {
		return nullptr;
	}
	MonsterType* mtype = g_monsters.getMonsterType(it->second);
	return (mtype ? mtype : nullptr);
