...
void Monster::onThink(uint32_t interval)
{
	...
	if (challengeMeleeDuration != 0) {
		...
		if (challengeMeleeDuration <= 0) {
			...
			targetDistance = mType->info.targetDistance;
			g_game.updateCreatureIcons(this);
		}
	}
...
std::vector<CreatureIcon> Monster::getIcons() const {
	...
	if (challengeMeleeDuration > 0 && mType->info.targetDistance > targetDistance){
		icon.id = CREATUREICON_TURNEDMELEE;
		tempIcons.push_back(icon);
	}

	if (varBuffs[BUFF_DAMAGEDEALT] < 100) {
		icon.id = CREATUREICON_LOWERDEALTDAMAGE;
		tempIcons.push_back(icon);
	}

	if (varBuffs[BUFF_DAMAGERECEIVED] > 100) {
		icon.id = CREATUREICON_HIGHERRECEIVEDDAMAGE;
		tempIcons.push_back(icon);
	}
	
	if (influencedLevel == 15) {
		icon.id = CREATUREICON_FIEND;
		tempIcons.push_back(icon);
	} else if (influencedLevel > 0) {
		icon.id = CREATUREICON_INFLUENCED;
		icon.value = influencedLevel;
		tempIcons.push_back(icon);
	}
	return tempIcons;
}

void Monster::setInfluencedLevel(int32_t value) {
	healthMax = mType->info.healthMax + mType->info.healthMax * (value*6/100.);
	health = mType->info.health + mType->info.health * (value*6/100.);
	influencedLevel = value;
