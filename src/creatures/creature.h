...
class Creature : virtual public Thing
{
    ...
    void setBuff(buffs_t buff, int32_t modifier) {
      varBuffs[buff] += modifier;
		...
		virtual void addIcon(CreatureIcon newIcon) {
			for (auto it = icons.begin(); it != icons.end(); it++) {
				if (it->id == newIcon.id && it->playerIcon == newIcon.playerIcon) {
					return;
				}
			}
		...
		virtual void removeIcon(CreatureIcon newIcon) {
			for (auto it = icons.begin(); it != icons.end(); it++) {
				if (it->id == newIcon.id && it->playerIcon == newIcon.playerIcon) {
					icons.erase(it);
					return;
