...
void Player::setTraining(bool value) {
	...
	this->statusVipList = VIPSTATUS_ONLINE;
...
{
	if (hasClient() && depotId && this->getLastDepotId() != -1) {
...
std::vector<CreatureIcon> Player::getIcons() const {
	std::vector<CreatureIcon> tempIcons = Creature::getIcons();
	return tempIcons;
}
...
void Player::onThink(uint32_t interval)
{
	...
	if (hasCondition(CONDITION_INFIGHT)) {
		int32_t momentum = getMomentumPercent();
		if (momentum > 0){
				momentumTicks += 1000;
				if (momentumTicks >= 2000) {
					momentumTicks = 0;
					if (uniform_random(1, 10000) <= momentum) {
						for (Condition* condition : conditions) {
							if (condition->getType() == CONDITION_SPELLCOOLDOWN) {
								uint32_t subId = condition->getSubId();
								int32_t ticks = condition->getTicks();
								ticks -= 2000;
								condition->setTicks(ticks);
								sendSpellCooldown(subId, std::max(0, ticks));
							} else if (condition->getType() == CONDITION_SPELLGROUPCOOLDOWN) {
								uint32_t subId = condition->getSubId();
								if (subId >= SPELLGROUP_SPECIAL) {
									int32_t ticks = condition->getTicks();
									ticks -= 2000;
									condition->setTicks(ticks);
									sendSpellGroupCooldown(static_cast<SpellGroup_t>(subId), std::max(0, ticks));
								}
							}
						}
...
int32_t Player::getOnslaughtPercent() const {
	int32_t result = 0;

	Item* weapon = getWeapon(true);

	if (!weapon)
		return result;
	

	if (Item::items[weapon->getID()].classification == 0)
		return result;

	uint8_t tier = weapon->getTier();
	if (tier == 0)
		return result;

	return result + getOnSlaughtValue(tier);
}

int32_t Player::getRusePercent() const {
	int32_t result = 0;

	if (!isItemAbilityEnabled(CONST_SLOT_ARMOR))
		return result;
	...
	if (Item::items[item->getID()].classification == 0)
		return result;
	...
	return result + getRuseValue(tier);
}

int32_t Player::getMomentumPercent() const {
	int32_t result = 0;

	if (!isItemAbilityEnabled(CONST_SLOT_HEAD))
		return result;
	...
	return result + getMomentumValue(tier);
}
