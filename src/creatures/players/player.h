...
class Player final : public Creature, public Cylinder
{
	public:
		...
		uint32_t getBossPoints() {
			return bossPoints;
		}
		void setBossPoints(uint32_t points) {
			bossPoints = points;
		}
		uint16_t getBossSlotRaceId(uint8_t slot) {
			return bossSlots[slot];
		}
		void setBossSlotRaceId(uint8_t slot, uint16_t bossId) {
			bossSlots[slot] = bossId;
		}
		uint16_t getBossSlotRemovals(uint8_t slot) {
			return bossSlotsRemovals[slot];
		}
		void setBossSlotRemovals(uint8_t slot, uint16_t total) {
			bossSlotsRemovals[slot] = total;
		}

		void addBosstiaryKillCount(uint16_t bossid, uint32_t amount)
		{
			uint32_t oldCount = getBosstiaryKillCount(bossid);
			uint32_t key = STORAGEVALUE_BOSSTIARYKILLCOUNT + bossid;
			addStorageValue(key, static_cast<int32_t>(oldCount + amount));
		}
		uint32_t getBosstiaryKillCount(uint16_t bossid) const
		{
			int32_t cp = 0;
			uint32_t key = STORAGEVALUE_BOSSTIARYKILLCOUNT + bossid;
			getStorageValue(key, cp);
		...
		int32_t getDust() const {
			return dust;
		}

		void setDust(int32_t value) {
			dust = std::min(255, std::max(0, value));
		}

		int32_t getMaximumDust() const {
			return maximumDust;
		}

		void setMaximumDust(int32_t value) {
			maximumDust = std::min(255, std::max(0, value));
		}

		int32_t getOnslaughtPercent() const;
		int32_t getRusePercent() const;
		int32_t getMomentumPercent() const;
 ...
 private:
    ...
    uint64_t asyncOngoingTasks = 0;
		...
		std::vector<uint16_t> bossSlots = {0, 0};
		std::vector<uint16_t> bossSlotsRemovals = {0, 0};
		...
		int32_t dust = 0;
		int32_t maximumDust = 100;
		uint16_t momentumTicks = 0;
