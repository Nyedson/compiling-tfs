...
void Game::loadBoostedBoss()
{
	...
	if (!result) {
		SPDLOG_WARN("[Game::loadBoostedBoss] - "
                    "Failed to detect boosted boss database. (CODE 01)");
		return;
	}
	...
	if (date == today) {
		...
		uint16_t oldid = result->getNumber<uint16_t>("bossid");
		std::map<uint16_t, std::string> bosslist = getBosstiaryList();
		uint16_t newid = 0;
		uint8_t k = 1;
		while (newid == 0 || newid == oldid || g_monsters.getMonsterType(name)->info.bosstiaryCategory != BOSSTIARY_CATEGORY_ARCHFOE) {
			uint16_t random = uniform_random(0, bosslist.size());
			for (auto it : bosslist) {
				if (k == random) {
					newid = it.first;
					name = it.second;
				}
		...
		if (!db.executeQuery(query.str())) {
			SPDLOG_WARN("[Game::loadBoostedBoss] - "
                        "Failed to detect boosted boss database. (CODE 02)");
			return;
		}
	}
	setBoostedBossName(name);
	SPDLOG_INFO("Boosted boss: {}", name);
}
...
Monster* Game::getRandomMonster() {
	...
	std::advance(it, rand() % monsters.size());
	return it->second;
}
...
void Game::updateSpoofPlayers()
{
  ...
  // SPDLOG_INFO(players.size() + " normal players online, " + spoofPlayers + " spoof players online");
...
void Game::updateCreatureIcons(const Creature* creature)
{
	SpectatorHashSet spectators;
	map.getSpectators(spectators, creature->getPosition(), true, true);
	for (Creature* spectator : spectators) {
...
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker, Creature* target, bool checkDefense, bool checkArmor, bool field)
{
	...
	static const auto sendBlockEffect = [this](BlockType_t blockType, CombatType_t combatType, const Position& targetPos) {
		if (blockType == BLOCK_RUSE) {
			addMagicEffect(targetPos, CONST_ME_RUSE);
	...
	bool fiendMessage = false;
	bool influencedMessage = false;
	...
	if (damage.primary.type != COMBAT_NONE) {
		...
		if (targetPlayer && damage.primary.type != COMBAT_HEALING) {
			int32_t ruseChance = targetPlayer->getRusePercent();
			if (uniform_random(1, 10000) <= ruseChance) {
				sendBlockEffect(BLOCK_RUSE, damage.primary.type, target->getPosition());
				return true;
			}
		}
		...
		if (attacker) {
			...
			if (attacker->getMonster()) {
				int32_t influencedLevel = attacker->getMonster()->getInfluencedLevel();
				if (influencedLevel > 0) {
					damage.primary.value += damage.primary.value * (influencedLevel * 6 / 100.);
					if (influencedLevel == 15)
						fiendMessage = true;
					else
						influencedMessage = true;
				}
			}
		}
	...
	if (fiendMessage) {
		...
		damage.exString += "fiend monster";
	}
	if (influencedMessage) {
		if (!damage.exString.empty()) {
			damage.exString += ", ";
		}
		damage.exString += "influenced monster";
	}
	if (canReflect) {
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage, bool isEvent /*= false*/)
{
	...
	if (damage.primary.value > 0) {
		...
		if (!isEvent && damage.origin != ORIGIN_NONE) {
			g_events->eventCreatureOnDrainHealth(target, attacker, damage.primary.type, damage.primary.value, damage.secondary.type, damage.secondary.value, message.primary.color, message.secondary.color, damage.exString);
		}
		...
		if (damage.origin != ORIGIN_NONE) {
			if (damage.onslaught) {
				...
				damage.exString += "onslaught";
				addMagicEffect(spectators, targetPos, CONST_ME_ONSLAUGHT);
			}
		}
		if (damage.critical) {
...
void Game::addBosstiaryList(uint16_t bossid, std::string name)
{
	auto it = BosstiaryList.find(bossid);
	if (it != BosstiaryList.end()) {
		return;
	}
	BosstiaryList.insert(std::pair<uint16_t, std::string>(bossid, name));
}

bool Game::removePlayerSlotBoss(Player* player, uint16_t slot, uint32_t cost)
{
	if (g_game.removeMoney(player, cost, 0, true)) {
		player->setBossSlotRaceId(slot, 0);
		player->setBossSlotRemovals(slot, player->getBossSlotRemovals(slot) + 1);
		return true;
	}
	return false;
}
...
void Game::playerBrowseMarket(uint32_t playerId, uint16_t spriteId, uint16_t tier)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
...
void Game::playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t spriteId, uint16_t amount, uint32_t price, bool anonymous, uint16_t tier) // Limit of 64k of items to create offer
{
	...
	if (type == MARKETACTION_SELL) {
      ...
      std::forward_list<Item *> itemList = getMarketItemList(it.wareId, amount, depotLocker, tier);
	...
	IOMarket::createOffer(player->getGUID(), static_cast<MarketAction_t>(type), it.id, amount, price, anonymous, tier);
...
void Game::playerAcceptMarketOffer(uint32_t playerId, uint32_t timestamp, uint16_t counter, uint16_t amount) // Limit of 64k of items to create offer
{
	...
	Player* offerPlayer = getPlayerByGUID(offer.playerId);
	if (!offerPlayer) {
		offerPlayer = new Player(nullptr);
		if (!IOLoginData::loadPlayerById(offerPlayer, offer.playerId)) {
			delete offerPlayer;
			return;
		}
	}

	if (player->getAccount() == offerPlayer->getAccount()) {
	...
	if (offer.type == MARKETACTION_BUY) {
      ...
      account.RemoveCoins(amount);
			...
			std::forward_list<Item*> itemList = getMarketItemList(it.wareId, amount, depotLocker, offer.tier);
			if (itemList.empty() && amount > 0) {
				return;
			}
      ...
      // if (stashminus > 0) {
		...
		if (it.id == ITEM_STORE_COIN) {
      account::Account account;
      account.LoadAccountDB(offerPlayer->getAccount());
    ...
    else if (it.stackable)
    {
      uint16_t tmpAmount = amount;
			while (tmpAmount > 0) {
				...
				Item* item = Item::CreateItem(it.id, stackCount);
				if (internalAddItem(offerPlayer->getInbox(), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
					delete item;
					break;
				}
    ...
    else
    {
      ...
      account.RegisterCoinsTransaction(account::COIN_ADD, amount,
			...
			offerPlayer->setBankBalance(offerPlayer->getBankBalance() + totalPrice);
			if (it.id == ITEM_STORE_COIN) {
        account::Account account;
        account.LoadAccountDB(offerPlayer->getAccount());
        account.RegisterCoinsTransaction(account::COIN_REMOVE, amount,
	...
	if (offerPlayer->isOffline()) {
		IOLoginData::savePlayer(offerPlayer);
		delete offerPlayer;
	}

	const int32_t marketOfferDuration = g_config.getNumber(ConfigManager::MARKET_OFFER_DURATION);

	IOMarket::appendHistory(player->getGUID(), (offer.type == MARKETACTION_BUY ? MARKETACTION_SELL : MARKETACTION_BUY), offer.itemId, amount, offer.price, offer.timestamp + marketOfferDuration, OFFERSTATE_ACCEPTEDEX, offer.tier);

	IOMarket::appendHistory(offer.playerId, offer.type, offer.itemId, amount, offer.price, offer.timestamp + marketOfferDuration, OFFERSTATE_ACCEPTED, offer.tier);
...
std::forward_list<Item*> Game::getMarketItemList(uint16_t wareId, uint16_t sufficientCount, DepotLocker* depotLocker, uint16_t tier)
{
	...
	do {
		...
		for (Item* item : container->getItemList()) {
			...
			if (!itemType.stackable && item->getTier() != tier)
				continue;
