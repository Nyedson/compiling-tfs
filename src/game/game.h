...
class Game
{
	public:
		...
		void playerBuyItem(uint32_t playerId, uint16_t spriteId, uint8_t count, uint16_t amount,
		...
		void playerBrowseMarket(uint32_t playerId, uint16_t spriteId, uint16_t tier);
		void playerBrowseMarketOwnOffers(uint32_t playerId);
		void playerBrowseMarketOwnHistory(uint32_t playerId);
		void playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t spriteId, uint16_t amount, uint32_t price, bool anonymous, uint16_t tier);
		...
		std::forward_list<Item*> getMarketItemList(uint16_t wareId, uint16_t sufficientCount, DepotLocker* depotLocker, uint16_t tier);
		...
		void addBosstiaryList(uint16_t bossid, std::string name);
		bool removePlayerSlotBoss(Player* player, uint16_t slot, uint32_t cost);
		const std::map<uint16_t, std::string>& getBosstiaryList() const { return BosstiaryList; }

		void setBoostedBossName(std::string name) {
			boostedBoss = name;
		}
		...
		void updateCreatureIcons(const Creature* creature);
	...
	private:
		...
		std::map<uint16_t, std::string> BosstiaryList;
