...
std::vector<std::pair<std::string, std::string>>
		...
		if (it.classification != 0) {
			...
			if (tier > 0) {
				...
				if (it.slotPosition & SLOTP_HEAD) {
					ss << (tier*tier + 38*tier + 1)/20. << "% Momentum";
				} else if (it.slotPosition & SLOTP_ARMOR) {
					std::vector<std::string> values = {"0.5", "1.03", "1.62", "2.28", "3", "3.78", "4.62", "5.52", "6.48", "7.51"};
					if (tier <= values.size())
						ss << values[tier - 1];
					ss << "% Ruse";
				} else if (it.weaponType > 0) {
					ss << (tier*tier + 8*tier + 1)/20. << "\% Onslaught";
				}
				ss << ')';
			}
	...
	if (it.classification != 0) {
		descriptions.emplace_back("Classification", std::to_string(it.classification));
	}
								 ...
								 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.classification != 0) {
		s << std::endl << "Classification: " << it.classification << " Tier: ";
		if (item) {
			uint16_t tier = item->getTier();
			s << tier;
			if (tier > 0) {
				...
				const ItemType& it = Item::items[item->getID()];
				if (it.slotPosition & SLOTP_HEAD) {
					s << (tier*tier + 38*tier + 1)/20. << "% Momentum";
				} else if (it.slotPosition & SLOTP_ARMOR) {
					std::vector<std::string> values = {"0.5", "1.03", "1.62", "2.28", "3", "3.78", "4.62", "5.52", "6.48", "7.51"};
					if (tier <= values.size())
						s << values[tier - 1];
					...
					s << (tier*tier + 8*tier + 1)/20. << "\% Onslaught";
				}
				s << ')';
			}
		} else {
